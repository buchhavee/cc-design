...
export function Hero() {
  ...
  return (
    <section
      ref={ref}
      id="top"
      className="relative h-svh md:h-auto md:min-h-svh flex items-center px-(--gutter) pt-20 pb-16 md:pt-24 md:pb-28 overflow-hidden bg-bg-inverse"
    >
      ...
      <div className="relative z-5 w-full max-w-(--max-page) mx-auto grid grid-cols-1 md:grid-cols-12 items-center gap-8 md:gap-10 lg:gap-16">
        ...
        <div className="md:col-span-7 lg:col-span-7 order-2 md:order-1">
          <div
            className="inline-flex items-center font-body text-xs font-medium tracking-[0.14em] uppercase text-fg-inverse/75 mb-4 md:mb-10 transition-opacity duration-600 delay-200"
          ...
          >
            <p className="m-0 font-body text-sm md:text-lg leading-[1.6] md:leading-[1.65] font-normal text-fg-inverse/80 max-w-[460px]">
              From concept to construction, we create durable, functional solutions for the maritime industry, developed through years of hands-on expertise.
            </p>
            <a
              href="#work"
              className="group inline-flex items-center gap-3.5 px-[22px] py-3.5 border border-fg-inverse/45 text-fg-inverse font-body text-xs font-medium tracking-widest uppercase no-underline cursor-pointer transition-[background,color,border-color] duration-240 hover:bg-bg hover:text-fg hover:border-bg"
            >
        ...
        >
          <Image
            src="/assets/havnebad-hero.jpeg"
            alt="Havnebad — aerial view of a harbor bath project by CC Design"
            fill
            sizes="(max-width: 768px) 60vw, (max-width: 1024px) 42vw, 520px"
            style={{ objectFit: "cover" }}
          />
          <div className="absolute left-3 right-3 bottom-3 sm:left-4 sm:bottom-4 sm:right-auto font-mono text-[10px] sm:text-[11px] tracking-[0.08em] uppercase text-fg-inverse/90 bg-[rgba(14,14,13,0.42)] px-2.5 py-1.5 backdrop-blur-md">
            Islands Brygges Havnebad Renovation · Featured Project
          </div>
        </div>
      </div>
      ...
      <div
        className="absolute left-1/2 -translate-x-1/2 bottom-5 md:bottom-9 flex flex-col items-center gap-2.5 md:gap-3 font-body text-[10px] tracking-[0.28em] uppercase text-fg-inverse/55 transition-opacity duration-700 delay-1100"
        style={{ opacity: visible ? 1 : 0 }}
      >
        <span>Scroll</span>
        <div className="relative w-px h-11 overflow-hidden bg-fg-inverse/20">
          <div
            className="absolute inset-x-0 top-0 h-1/3 bg-fg-inverse/90"
            style={{ animation: "scrollHint 2.4s var(--ease-out-quart) infinite" }}
          />
        </div>
      </div>
    </section>
